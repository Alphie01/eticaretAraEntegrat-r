...
class SuratCargoTracker {
  constructor() {
    this.baseURL =
      process.env.SURAT_CARGO_API_URL || "https://ws.suratkargo.com.tr";
  ...
  async trackShipment(trackingNumber) {
    try {
      ...
      logger.error(
        `Sürat Kargo tracking error for ${trackingNumber}:`,
        error.message
      );
      throw error;
    }
  ...
  async trackShipmentDetail(trackingNumber) {
    try {
      ...
      logger.error(
        `Sürat Kargo detail tracking error for ${trackingNumber}:`,
        error.message
      );
      throw error;
    }
  ...
  async cancelShipment(trackingNumber, reason = "Customer request") {
    try {
      ...
      logger.error(
        `Sürat Kargo cancellation error for ${trackingNumber}:`,
        error.message
      );
      throw error;
    }
  ...
  validateShipmentData(data) {
    ...
    if (
      data.receiverPhone &&
      !/^(\+90|0)?[5][0-9]{9}$/.test(data.receiverPhone.replace(/\s/g, ""))
    ) {
  ...
  processShipmentResult(result) {
    if (!result.success) {
      throw new Error(
        `Shipment creation failed: ${result.message || "Unknown error"}`
      );
    }
    ...
    try {
      ...
      return {
        ...
        rateLimits: {
          ...
          resetTime: new Date(
            this.rateLimiter.lastReset + this.rateLimiter.resetInterval
          ),
        },
        features: {
          createShipment: true,
